...
/*
  ...
  Argument: function(confirmed, guiInventory) returning whether to close the inventory
*/
@AutoConstruct
public class ConfirmationGui extends AGui<BiFunction<Boolean, Inventory, Boolean>> {
  ...
  @Override
  protected boolean closed(GuiInstance<BiFunction<Boolean, Inventory, Boolean>> inst) {
    if (!madeSelection.remove(inst.getViewer()))
      inst.getArg().apply(false, inst.getInv());
    return false;
  }

  @Override
  protected boolean opening(Player viewer, GuiInstance<BiFunction<Boolean, Inventory, Boolean>> inst) {
    ...
    inst.fixedItem(11, i -> (
      ...
      if (inst.getArg().apply(true, inst.getInv()))
        inst.close();
    }, null);
    ...
    inst.fixedItem(15, i -> (
      ...
      madeSelection.add(inst.getViewer());
      if (inst.getArg().apply(false, inst.getInv()))
        inst.close();
    }, null);
