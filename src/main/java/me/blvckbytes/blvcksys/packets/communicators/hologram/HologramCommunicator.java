...
public class HologramCommunicator implements IHologramCommunicator {
  ...
  public Entity createLine(Player p, Location loc, String line) {
    try {
      Location shifted = shiftLocation(loc);
  ...
  @Override
  public void teleportLine(Player p, Entity handle, Location loc) {
    try {
      Location shifted = shiftLocation(loc);
  ...
  @Override
  public void moveLine(Player p, Entity handle, Location to) {
    try {
      Location toShifted = shiftLocation(to);

      // Get the last location of this line
      Location prev = lastLocations.get(handle);
      if (prev == null)
        throw new IllegalStateException("Unknown hologram line with id=" + handle.getEntityId());
      ...
      lastLocations.put(handle, to.clone());
